class PhaseTwoController < ApplicationController
    ...
    if logged_in?
      ...
      if authenticated_project?
        ...
        if @task.nil?
          ...
          erb :'/user_is_logged_in/phase_2/phase_2_first_iteration'
        end
      else
        redirect '/no_access'
      end
  ...
  get '/phase_2_question_time/:project_id/?:task_score_id?' do
    if logged_in?
      ...
      @project = Project.find(params[:project_id])
      if authenticated_project?
        @phantom = TaskScore.find_by(params[:task_score_id]) #this is actually a useless line of code that I haven't deleted yet ... letting it hang around though because I want to have it for future reference about invoking optional route parameters
        ...
        unless @task.nil?
          ...
          erb :"/user_is_logged_in/phase_2/phase_2_question_time"
        else
